Feature: UI:CountryUser - Card Control Profile Scenarios from Cards module

  @UI-CardControlProfile
  Scenario: OMV-885 User validate search criteria should be retained in card control profile module
  ...
  @UI-CardControlProfile
  Scenario: OMV-885 Add new private card control profile
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
...
#    Then User validate all stepper values in Review page of "cardControlProfile"
  ...
  @UI-CardControlProfile
  Scenario: OMV-892 Edit new private card control profile for country user
    Given User is on Login page and validate WEX logo text "OMV", for scenario "TC01 : OMV-892 Country user edit a cardControlProfile for cardOffer"
...
#    And User validate "snackbar-text" message "Day and time restrictions has been updated" in "pricingRules" module
    And User click "Next" button using Java Script executor which is present at position "1" using tag name "span"
    And User validate velocity limits in "cardControlProfile" page for "Edit" form
    And User click on "Review" button
    Then User validate all stepper values in Review page of "cardControlProfile"
    And User click on "Submit" button
...
#    And User validate card control profile added or updated in the database based on action "Edit"
  ...
  @UI-CardControlProfile
  Scenario: OMV-885 User validate error messages for card Product and card control profile name
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
    Then User get account number which is "notEligible" for create a new cardControlProfile
    ...
    Then User handle "cardControlProfileName" section for module "CardControlProfile" and perform action "Edit"
    And User click "Next" button using Java Script executor which is present at position "1" using tag name "span"
    Then User validate presence of "Enter a different name" field with "mav-error" tag

  @UI-CardControlProfile
  Scenario Outline: OMV-884 : Validate Search and Default field values in Card control profile module after pinning an account
    Given User is on Login page and validate WEX logo text "OMV", for scenario "<Scenario>"
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
    Then User select "Select account" option using tag name "div" from context picker if logged in user "<UserName>" has access to more than one customer
    And User select client and country based on logged in user "CountryUserName"
    And User enter accountnumber which has records of "CardControlProfile"
    And User click on "Search" button
#    Then User validate based on the search criteria the record count displayed in module "accounts" or not for field "accountNumber" based on field value
    ...
    And User click on " Search " button
    And User validate "Default" omv Card Control Profile section field values "Profilename,Cardoffer,Products,Type,AcquiringClient,Networks,Sites" with database based on searched field "<fieldNameOfCardProfilesearch>"

    Examples:
      | Scenario                                                                                                                                                                          | UserName        | fieldNameOfCardProfilesearch | CardProfilesearchFieldBehavior |
      | TC-01 : OMV-884 : Validate Search and Default field values in Card control profile module after pinning an account by logging in using csrClient and search using CardControlName | CountryUserName | cardControl                  | text                           |
#      | TC-02 : OMV-884 : Validate Search and Default field values in Card control profile module after pinning an accountby logging in using csrClient and search using Products         | CountryUserName | products                     | dropDown                       |

  ##Export list of card control profiles at single account level
  Scenario: TC01: OMV-893 Validate Export sheet data of Cardcontrol profile module after pinning an account
    Given User is on Login page and validate WEX logo text "OMV", for scenario "<Scenario>"
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
    Then User select "Select account" option using tag name "div" from context picker if logged in user "<UserName>" has access to more than one customer
  ...
  Scenario: OMV-1746 :Country User : Validate Expanded section field values in Card Control profile module with out pinning an account for Purchase,Refund,rebate transaction
    Given User is on Login page and validate WEX logo text "OMV", for scenario "<Scenario>"
    And User click on button "Home" using span text
    When User click on the "menu" "Cards"
