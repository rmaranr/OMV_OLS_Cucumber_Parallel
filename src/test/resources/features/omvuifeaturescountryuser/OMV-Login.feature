Feature: This feature contains Login related scenarios

  @UI-Login
  Scenario Outline: Login to the OMV application using valid credentials and verify the user is navigated to Home page or not
...
#    Then User validate Home page has been loaded
    ...
    Examples:
      | UserName        | PassWord        |
      | CountryUserName | CountryPassword |

  @UI-Login
  Scenario Outline: User enter valid user name and invalid password then verify error message
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
  ...
  @UI-Login
  Scenario: Country User : OMV-2254 User validate Help and contact us links available and getting navigated to corresponding website before and after login to applicaiton
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
