Feature: UI:CountryUser - Document setup scenarios from Accounts module


    ########################### Report setup scenarios #######################
  @UI-DocumentSetup
  Scenario: TC-01 : OMV-1020 : Country User : Validate the configured reports for the selected account
  ...
  @UI-DocumentSetup
  Scenario: TC-02 : OMV-1020 : Country User : User validate actions are visible based on status of report
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
    Then User get an "Active" account number which "having" Document setup for user "OMV"
  ...
  @UI-DocumentSetup
  Scenario: TC-01 : OMV-1021 : Country User : User create a new report setup for account which does not contains report setups and validate 'Cancel' button functionality in add new report setup page
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
...
#    And User enter all mandatory fields based on corresponding look up values and field should be disabled if it has only one value
  ...
  @UI-DocumentSetup
  Scenario: TC01 : Country User : OMV-1022 Validate edit functionality of report configuration
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
    Then User get an "Active" account number which "having" Document setup for user "OMV"
  ...
  Scenario: TC01: Country User :OMV-1023 Validate approve and decline functionality in Report setup module
    Given User is on Login page and validate WEX logo text "OMV", for scenario ""
    And User click on button "Home" using span text
    And User make sure account is not pinned for logged in user
