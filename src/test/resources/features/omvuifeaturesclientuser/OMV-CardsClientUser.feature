Feature: UI:ClientUser - Cards module

  @UI-Cards
  Scenario Outline: OMV-780,781,782,783 : ClientUser : Validate Default and Expanded section field values in Cards module with out pinning an account
...
#    And User validate "DeliveryAddress" omv section field values "cardAddress,pinAddress,mailIndicator" with database based on searched field "<fieldNameOfCardsearch>" based on user "<UserName>"
    ...
    Examples:
      ...
      | TC-13 : OMV-780,781,782 : Validate Cards Default and Expanded section field values with out pinning an account by logging in using csrClient and search using status                              | ClientUserName | status                | text                    |
  ...
  @UI-Cards
  Scenario Outline: OMV-780,781,782,784,772 : ClientUser : Validate Default and Expanded section field values in Cards module after pinning an account
    Given User is on Login page and validate WEX logo text "OMV", for scenario "<Scenario>"
    And User click on button "Home" using span text
...
#    And User validate "DeliveryAddress" omv section field values "cardAddress,pinAddress,mailIndicator" with database based on searched field "<fieldNameOfCardsearch>" based on user "<UserName>"
    ...
    Examples:
      | Scenario                                                                                                                                                                  | UserName       | fieldNameOfCardsearch | CardsearchFieldBehavior |
      | TC-08 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using Customernumber | ClientUserName | customerNumber        | text                    |
      | TC-09 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using IssuingCountry | ClientUserName | issuingCountry        | text                    |
      | TC-10 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using Drivername     | ClientUserName | driverName            | text                    |
      | TC-11 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using Licenseplate   | ClientUserName | licensePlate          | text                    |
      | TC-12 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using Costcentre     | ClientUserName | costCentreCode        | text                    |
      | TC-13 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using status         | ClientUserName | status                | text                    |
      | TC-14 : OMV-780,781,782 : Validate Cards Default and Expanded section field values after pinning an account by logging in using csrClient and search using Cardnumber     | ClientUserName | cardNumber            | text                    |
  ...
  @UI-Cards
  Scenario Outline: TC-01 : OMV-864,872,873,1091,1188 Client User : Order a card using self select identification method
...
#    Then User validate all stepper values in Review page in form "clone"
    ...
    Examples:
      | Scenario                                                                                                                                      | UserName       | identificationMethod | cardControlRadiobuttonName | actionOfCardControlPopUp | noOfCard              | isAlternateAddressRequired | locationRestrictionScenario | cardReissueTypeCID | timeLimits              |
      | TC01 : OMV-888 Client User Clone a card with identification method "Self select pin" and "Existing profile" for new card control profile      | ClientUserName | opposite             | existing                   | newPrivate               | reIssueWithDiffCardNo | Yes                        | bothValues                  | 5901               | Wednesday,Thursday      |
